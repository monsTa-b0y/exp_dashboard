# # expenditure_analysis_dashboard.py
# # To run this: 
# # 1. Install required libraries: pip install streamlit pandas plotly
# # 2. Run the script: streamlit run expenditure_analysis_dashboard.py
...
#     # Assume columns: 'Date', 'transaction_details', 'Amount', 'Tags'
...
#         st.stop()
#     except ValueError:
#         st.error("Invalid Date format in 'Date' column.")
#         st.stop()
...
#     # Filter for expenditures (assuming 'Amount' is positive for expenses; adjust if needed)
#     df = df[df['Amount'] > 0]  # Or use df['Amount'].abs() if signs vary
...
#     selected_Tags = st.sidebar.multiselect("Select Tags", all_Tags, default=all_Tags)
#     if selected_Tags:
#         df_filtered = df_filtered[df_filtered['Tags'].isin(selected_Tags)]
...
#     st.metric("Total", f"₹{total_exp:,.2f}")
    
#     # Visual 1: Pie chart of expenditures by tag
#     st.subheader("Expenditures by Tag (Pie Chart)")
#     tag_sum = df_filtered.groupby('Tags')['Amount'].sum().reset_index()
#     fig_pie = px.pie(tag_sum, values='Amount', names='Tags', title="Breakdown by Tags")
#     st.plotly_chart(fig_pie, use_container_width=True)
    
#     # Visual 2: Bar chart of expenditures by tag
#     st.subheader("Expenditures by Tag (Bar Chart)")
#     fig_bar = px.bar(tag_sum, x='Tags', y='Amount', title="Bar Chart by Tags")
#     st.plotly_chart(fig_bar, use_container_width=True)
    
#     # Visual 3: Time series line chart of daily expenditures
#     st.subheader("Daily Expenditures Over Time")
#     daily_sum = df_filtered.resample('D', on='Date')['Amount'].sum().reset_index()
#     fig_line = px.line(daily_sum, x='Date', y='Amount', title="Daily Total Expenditures")
#     st.plotly_chart(fig_line, use_container_width=True)
    
#     # Visual 4: Scatter plot of individual transactions
#     st.subheader("Individual Transactions (Scatter Plot)")
#     fig_scatter = px.scatter(df_filtered, x='Date', y='Amount', color='Tags', 
#                              hover_data=['Transaction Details'], title="Transactions by Date and Amount")
#     st.plotly_chart(fig_scatter, use_container_width=True)
    
#     # Table of top transactions
#     st.subheader("Top 10 Transactions")
#     top_tx = df_filtered.sort_values('Amount', ascending=False).head(10)
#     st.dataframe(top_tx[['Date', 'Transaction Details', 'Amount', 'Tags']])
# else:
